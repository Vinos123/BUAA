...
void AD9854_IO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6| GPIO_Pin_7| GPIO_Pin_5| GPIO_Pin_8| GPIO_Pin_2; //PA口初始化
...
{
//	dat1= dat|(addr<<8);//地址左移八位（低八位为数据），若将地址设置为低八位则将数据左移6位即可
	dat1 = (dat <<6) | (addr);//此时相当于数据为PC口的高八位，地址为PC口的低六位
	AD9854_AdrBus=(dat1&0x3fff)|((dat1^0x3fff)<<16);//给相当于给GPIOC赋值
