function [parameter,index] = hough_circle(BW,step_r,step_angle,r_min,r_max,p)  
...
[rows,columns] = find(BW);%查找非零元素的行列坐标  
...
for i=1:ecount  
    for r=1:size_r %半径步长数  
        for k=1:size_angle %按一定弧度把圆几等分  
            ...
            b = round(columns(i)-(r_min+(r-1)*step_r)*sin(k*step_angle));  
            if(a>0&&a<=m&&b>0&&b<=n)  
...
for i=1:ecount  
    for k=1:length  
        par3 = floor(index(k)/(m*n))+1;  %r
        par2 = floor((index(k)-(par3-1)*(m*n))/m)+1;  %y
        par1 = index(k)-(par3-1)*(m*n)-(par2-1)*m;  %x
        if((rows(i)-par1)^2+(columns(i)-par2)^2<(r_min+(par3-1)*step_r)^2+5&&...  
(rows(i)-par1)^2+(columns(i)-par2)^2>(r_min+(par3-1)*step_r)^2-5)  
              hough_circle(rows(i),columns(i)) = true;   %检测的圆  
        end  
    end  
end                 
...
while size(circleParaXYR,1) >= 1  
   ...
   if size(circleParaXYR,1)>1       
     for k=2:size(circleParaXYR,1)  
      ...
      parameter=[parameter;c1,c2,c3]; %保存各个圆的圆心和半径的值  
      circleParaXYR=XYR;  
end  
