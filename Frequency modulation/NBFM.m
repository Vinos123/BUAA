...
s_t=cos(2*pi*f_c*t+int(m_t,t));%已调信号

t=t_s:t_s:0.4;%信号时长为4个调制信号的周期
%代入具体数值
m_t_1=eval(m_t);%调制信号代入数值
m_c_1=eval(m_c);%载波信号代入数值
s_t_1=eval(s_t);%已调信号代入数值
S_t=fftshift(fft(s_t_1,N))*t_s;%已调信号频谱;
S_t_p=S_t.*conj(S_t);%已调信号功率谱;
%希尔伯特变换求已调信号的包络；
s_t_ah=hilbert(s_t_1);
s_t_a=abs(s_t_ah);
f=linspace(-f_s/2,f_s/2,N);%离散频率转化为连续频率;%四个信号周期因此除以4
